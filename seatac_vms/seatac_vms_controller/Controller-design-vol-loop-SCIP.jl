...
loop_run=20;
X_runs=zeros(4,17,loop_run);   # store states over all the loop runs
U_runs=zeros(2,16,loop_run);   # store inputs over all the loop runs
for loop_no=1:loop_run  # no of times to run the loop (then take average over them)
   IP=11498; # initial time
   Tf=16 # time-horizon (each step is 15 minute slot)
   To=16 # receding horizon length
   ...
   num_state=4 # number of states in the model (DF;DS;AF;AS)
   crit_dep=30 # critical speed for departure
   crit_arr=35 # critical speed for arrivals
   X_final=zeros(num_state,To+1); # storing output state values over time
   U_final=zeros(num_in,To);   # storing control inputs over time
   X_final[:,1]=Array(SF_data[IP,3:6]); # initial state
   ...
   for run_no=1:To   # running a loop over the receding horizon
      ...
      @constraint(JM,Crit_arr_vel .<= X[4,2:Tf+1]/crit_arr)

      #objective is to keep critical speed (departure and arrival) around 1 + with minimal controls
      ...
      # adding random noise based on regression fit of A and B matrices to obtain "actual" model
      X_final[:,run_no+1]=X_value[:,2]+[rand(Uniform(-34.6,38.5));rand(Uniform(-2.9,3.4));rand(Uniform(-43.3,46.5));rand(Uniform(-3.5,3.9))]; # storing values
      U_final[:,run_no]=U_value[:,1];

   end
   X_runs[:,:,loop_no]=X_final;
   U_runs[:,:,loop_no]=U_final;
end
